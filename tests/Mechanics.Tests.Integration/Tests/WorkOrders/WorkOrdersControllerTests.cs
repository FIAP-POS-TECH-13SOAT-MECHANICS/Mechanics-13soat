...
public class WorkOrdersControllerTests
{
    ...
    [TestMethod]
    public async Task GetAverageServiceTime_ShouldReturnSumOfServiceCatalogAverageTimes()
    {
        var client = await TestProperties.Factory.GetAuthenticatedClient(RoleNames.Attendant);

        var customerId = Guid.NewGuid();
        var vehicleId = Guid.NewGuid();
        var serviceIdOne = Guid.NewGuid();
        var serviceIdTwo = Guid.NewGuid();
        ...
        using (var scope = TestProperties.Factory.Services.CreateScope())
        {
            ...
            db.Vehicles.Add(new Vehicle
            {
                ...
                Color = VehicleColor.Black,
                Year = "2021",
                LicensePlate = new LicensePlate("AVG0001"),
                Chassis = "CHAVG",
                OwnerId = customerId,
            });

            db.ServiceCatalog.AddRange(new ServiceCatalog
            {
                Id = serviceIdOne,
                Name = "Diagnóstico avançado",
                Description = "Executa diagnóstico detalhado",
                BasePrice = 150m,
                AverageTime = 30,
                Status = ServiceCatalogStatusType.Active,
            },
                new ServiceCatalog
                {
                    Id = serviceIdTwo,
                    Name = "Troca de óleo",
                    Description = "Substituição completa do óleo",
                    BasePrice = 120m,
                    AverageTime = 45,
                    Status = ServiceCatalogStatusType.Active,
                });
        ...
        {
            VehicleId = vehicleId,
            ReportedProblem = "Luz de alerta acesa",
            ServiceCatalogIds = new[] { serviceIdOne, serviceIdTwo }
        };

        var createResponse = await client.PostAsJsonAsync("/api/work-orders", createRequest,
            TestContext.CancellationTokenSource.Token);

        Assert.AreEqual(HttpStatusCode.Created, createResponse.StatusCode);

        var createdBody =
            await createResponse.Content.ReadFromJsonAsync<Dictionary<string, Guid>>(TestContext.CancellationTokenSource.Token);
        ...
        var workOrderId = createdBody["id"];

        var averageResponse = await client.GetAsync($"/api/work-orders/{workOrderId}/services/average-time",
            TestContext.CancellationTokenSource.Token);

        Assert.AreEqual(HttpStatusCode.OK, averageResponse.StatusCode);

        var body = await averageResponse.Content
            .ReadFromJsonAsync<GetWorkOrderAverageTimeResponse>(TestContext.CancellationTokenSource.Token);

        Assert.IsNotNull(body);
        Assert.AreEqual(workOrderId, body.WorkOrderId);
        Assert.AreEqual(75, body.TotalAverageTime);
    }
